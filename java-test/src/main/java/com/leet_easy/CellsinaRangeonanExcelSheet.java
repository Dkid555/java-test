...
public class CellsinaRangeonanExcelSheet {
    public static List<String> cellsInRange(String word) {
        List<String> result = new ArrayList<>();
        for (char letter = word.charAt(0); letter <= word.charAt(3); letter++) {
            for (int position = word.charAt(1) - '0'; position <= word.charAt(4) - '0'; position++) {
                result.add(letter + String.valueOf(position));
            }
        }
        return result;
    }
