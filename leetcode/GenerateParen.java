...
public class GenerateParen {
    public static void Gen(List<String> fin, String s, int left, int right){
        ...
        if(left > 0) Gen(fin, s + "(",left - 1, right);// if there are still left parentheses to add // add a left parenthesis and recursively call the function
        if(right > left) Gen(fin, s + ")",left, right - 1); // if there are still right parentheses to add and there are more left parentheses than right parentheses in the current string
