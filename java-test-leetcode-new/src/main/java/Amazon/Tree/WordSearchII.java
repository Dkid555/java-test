    ...
    public static List<String> findWords(char[][] board, String[] words) {
        ...
        for (int row = 0; row < board.length; ++row){
            for(int column = 0; column < board[row].length;++column){
                DFS(trie, board, row, column, result);
            }
        }
        return new ArrayList<>(result);
    }

    private static void DFS(Trie node,char[][] board , int row, int column, Set<String> result){
        if(row < 0 || row >= board.length || column < 0 || column >= board[row].length
                || board[row][column] == '#' || node.childrens[board[row][column] - 'a'] == null){
            return;
        }
        if(node.childrens[board[row][column] - 'a'].word != null){
            result.add(node.childrens[board[row][column] - 'a'].word);
        }
        node = node.childrens[board[row][column] - 'a'];
        char temp = board[row][column];
        board[row][column] = '#';
        DFS(node, board, row + 1, column, result);
        DFS(node, board, row - 1, column, result);
        DFS(node, board, row, column + 1, result);
        DFS(node, board, row, column - 1, result);
