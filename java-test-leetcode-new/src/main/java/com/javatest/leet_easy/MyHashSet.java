...
public class MyHashSet {
    ...
    public MyHashSet() {
        bucket = new NodeSet[16];
    }

    public void add(int key) {
        if (bucket[(bucket.length - 1) & key] == null) {
            bucket[(bucket.length - 1) & key] = new NodeSet(null, key);
        } else {
            NodeSet root = bucket[(bucket.length - 1) & key];
            ...
            if (root.key != key) {
                root.next = new NodeSet(null, key);
            }
//            root.next = (root.key != key) ? new NodeSet(null, key) : null;
    ...
    public void remove(int key) {
        if (bucket[(bucket.length - 1) & key] != null) {
            ...
            if (root == null) return;
            if (root.key == key) {
                bucket[(bucket.length - 1) & key] = root.next;
            } else {
                NodeSet temp = root;
...
class NodeSet {
    public NodeSet(NodeSet next, int key) {
        this.next = next;
        this.key = key;
    }
