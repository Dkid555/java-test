    ...
    public int maximumLength(String s) {
        ...
        for (char c : s.toCharArray()) {            // compute all the cont. char count and add it to the corresponding char in the map
            if (c == last)
                ...
                if (!ump.containsKey((char) last))
                    ump.put((char) last, new ArrayList<>());
        ...
        for (Map.Entry<Character, List<Integer>> entry : ump.entrySet()) {  // For each char evaluate the max 3 occuarance count and check for possible max triplate count
            ...
            if (l1 == l2 && l1 > l3)
                l2--;      // This is to handle the case when the largest 3 occurances are 2, 2, 1
            if (l1 + l2 + l3 >= 3)
                ans = Math.max(ans, Math.max(l1 - 2, l2));  //max triplet can be either largest - 2 or the second largest
        }
